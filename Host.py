...
class Host:
	...
	def get_osname( self, osname):
		'''return a osmatch object'''
		for osmatch_nodes in self.host_node.getElementsByTagName('osmatch'):
			if osmatch_nodes.getAttribute('name') == osname :
				os = osmatch.osmatch(osmatch_nodes)
				return os
		return None
